...
const defaultBreakpoints = [500, 700, 1000].map(n => n + 'px')

export const defaultTheme = {
  space: [0, 4, 8, 16, 32, 64, 128, 256, 512].map(n => n + 'px'),
  fontSizes: [12, 14, 16, 20, 24, 32, 48, 64, 72].map(n => n + 'px'),
}
...
export const css = args => (props = {}) => {
  ...
  for (const key in styles) {
    ...
    if (key === 'variant') {
      const variant = css(get(theme, val))(theme).result
      result = { ...result, ...variant }
      continue
    }

    if (val && typeof val === 'object') {
        // const cssObject = css(val)(theme).result;
        // if (key === 'whileHover') {
            // other[key] = cssObject;
            other[key]=val;
        // } else {
        //     result[key] = cssObject;
